 ...
 * @Last Modified time: 2019-06-10 10:22:40
 * description:
...
class Category extends Component {
    ...
    constructor(props) {
        super(props)
        this.state = {
            selectedTags: this.props.activeKey ? [this.props.content[this.props.activeKey - 1]] : [],//激活状态的标签
    ...
    handleChange  = (tag, checked,index) =>{
        if(this.props.handleIndex){
            this.props.handleIndex(index+1)
        }
    ...
    render() {
        ...
        const { defaultActiveKey, ...otherProps } = this.props;
                    ...
                    <Col className='category-box'>
                        ...
                        <div className='category-content' ref={this.categoryContent}> {tagsFromServer.map((tag, index) => (
                            <CheckableTag
                                ...
                                key={tag[keyName]}
                                checked={
                                    !selectedTags.length ? (defaultActiveKey ? isEqual(tag,defaultActiveKey) : false)
                                        : (selectedTags.indexOf(tag) > -1 || this.props.content[this.props.activeKey-1] == tag)}
