...
class Maintenance{
	...
	@observable calendarSearchOptions={
			planNo: '',
			pageSize: 10,
			pageIndex: 1,
	}
    ...
    @action.bound async getMaintenanceList(params) {
		...
		console.log(toJS(this.maintenanceList));
	}
	...
	@action async getDateData(date){
		// this.queryCalendarPageList();
		const tempData=await this.queryCalendarPageList();
			...
			try {
				...
				this.maintenanceList.map(value =>{
					if(value.currentDate === date){
						...
						console.log(this.chosenDateRowName,this.chosenDateRow)
					}
				})
			} catch (error) {
				console.log(error);
	...
	queryCalendarPageList = () => {
		this.getMaintenanceList({
			...this.calendarSearchOptions
		});
	};
