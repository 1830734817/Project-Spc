...
class Home {
  ...
  @observable updateVis = false;                      //重命名弹窗是否可见
  @observable newNameKey = '';                        //重命名元素的key
...
@action async getSideMenuList(){
  ...
  @action.bound setUpdateVis(visible) {
    this.updateVis = visible;
  }
  ...
  /* 重命名 */
  @action async rename(params){
    try {
			let res = await services.posts('saveRename')({
        ...params,
        key:this.newNameKey
      });
			if (isDataExist(res)) { //后端应更新完数据库后再返回
  ...
  /* 删除 */
  @action async sideMenuDelete(){
    try {
			let res = await services.posts('deleteSideMenu')({key:this.newNameKey}); //todo:变量名
			if (isDataExist(res)) {
        this.querySideMenu();
  ...
  @action.bound setNewNameKey(params){
    this.newNameKey = params;
  }
