...
| autoClearSearchValue | 是否在选中项后清空查询框，只在 `mode` 为 `multiple` 或 `tags` 时有效。 | boolean | true | 3.10.0 |
| autoFocus | 默认获取焦点 | boolean | false |  |
| dataSource | 下拉菜单option的数据源，每一项数据的格式如：{ label:'选项一',value:1 } | Array | - |  |
...
| optionFilterProp | 查询时过滤对应的 option 属性，如设置为 children 表示对内嵌内容进行查询。[示例](https://codesandbox.io/s/antd-reproduction-template-tk678) | string | value |  |
| optionLabelProp | 回填到选择框的 Option 的属性值，默认是 Option 的子元素。比如在子元素需要高亮效果时，此值可以设为 `value`。 | string | `children` （combobox 模式下为 `value`） |  |
| placeholder | 选择框默认文字 | string | - |  |
| showArrow | 是否显示下拉小箭头 | boolean | true | 3.2.1 |
| showSearch | 使单选模式可查询 | boolean | false |  |
