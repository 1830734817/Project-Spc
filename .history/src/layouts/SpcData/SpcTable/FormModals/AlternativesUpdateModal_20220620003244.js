...
class AlternativesUpdateModal extends Component {
    ...
    render() {
        return (
            <Modal title='设置预设值' visible={this.store.alternativesUpdateModalVis}
    ...
    //表单提交成功
    onFinish = (values) => {
        const {tempColInfo,colInfoIndex} = this.store;
        const key = tempColInfo[colInfoIndex].colValues.length+1;
        values.key = key;
        console.log(values);
        this.store.tempColInfo[this.store.colInfoIndex].colValues.push(values);
        console.log(this.store.tempColInfo[this.store.colInfoIndex].colValues);
        this.store.setAlternativesUpdateModalVis(false);
    };
