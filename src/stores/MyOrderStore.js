 ...
 * @LastEditors: shoen
 * @LastEditTime: 2021-11-29 23:46:37
 */
...
class MyOrder {
	...
	@observable orderStatus = 'add';              // 记录计划弹窗状态
	@observable orderModalVis = false;
	@observable orderCurrentRe = {};              // 编辑当前的record
	...
	@observable showDateTime=null;

	@observable typeList=[];
	@observable isReplenishmentOrderList=[];

	type=['销售订单','制造订单','采购订单','库存'];
	isReplenishmentOrder=['录入订单','补充订单'];
	...
	@action.bound async getOrderList(params) {
		...
		try {
			let res = await services.gets('getOrderList')({
				...omit(this.orderPageInfo, 'total'),
				...params
			});
			this.isLoading = false;
			if (isDataExist(res)) {
				this.orderList = res.data.data.items;
				this.orderPageInfo = res.data.data.page;        // pageIndex: 1, pageSize: 10, total: 17823
			}
		} 
		catch (error) {
			console.log(error);
	...
	@action async save(params) {
		console.log("SAVE");
		try {
			let res = await services.posts('saveOrder')(params);
			if (isDataExist(res)) {
				this.queryList();
				return res
			}
	...
	@action async deleteOrder(params, type) {
		try {
			let req = type === 'batch' ? 'batchRemove' : 'removeOrder';
	...
	@action.bound showModal(visible) {
		this.orderModalVis = visible;
	}
	
	@action clearModal() {
		this.orderModalVis = false;
		this.orderCurrentRe = {}
	}
