...
@action async getSideMenuList(){
  ...
  /* 新建表格 */
	@action async createTable(params) {
		try {
			let res = await services.posts('saveCreateTable')(params);
  ...
  /* 新建分组 */
	@action async createClass(params) {
		try {
			let res = await services.posts('saveCreateClass')(params);
			if (isDataExist(res)) { //后端应更新完数据库后再返回
        //刷新左侧菜单
				this.querySideMenu();
				return res
  ...
  querySideMenu = () => {
	...
	@action.bound setIsClass(visible) {
		this.isClass = visible;
	}
