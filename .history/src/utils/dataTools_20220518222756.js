...
/* 校验当前menu选择 */
export function checkCurrentMenu({ menuObj, actionId }) {
	let currentMenu = [],
		crumbsList = [];
	/* 一级菜单查询 */
	...
	try {
		if (isEmpty(currentMenu)) {
			...
			for (let lv2 of menuObj) {
				if (lv2.leafMenuModels) {
					if (lv2.leafMenuModels.length > 0) {
						currentMenu = lv2.leafMenuModels.filter(
							leaf => leaf.id === actionId
						);
						if (!isEmpty(currentMenu)) {
							...
							for (let lv3 of lv2.leafMenuModels) {
								if (lv3.leafMenuModels) {
									currentMenu = lv3.leafMenuModels.filter(
										leaf => leaf.id === actionId
									);
									if (!isEmpty(currentMenu)) {
										crumbsList.push({
...
/* 校验当前sideMenu选择 */
export function checkCurrentSideMenu( menuObj, actionId ) {
	let currentSideMenu = [];
	/* 一级菜单查询 */
	console.log(menuObj, actionId)
	currentSideMenu = menuObj.filter(leaf => leaf.id === actionId);
	try {
		if (isEmpty(currentSideMenu)) {
			/* 二级菜单查询 */
			for (let lv2 of menuObj) {
				console.log(lv2.childMenu)
				if (lv2.childMenu) {
					if (lv2.childMenu.length > 0) {
						currentSideMenu = lv2.childMenu.filter(
							leaf => leaf.id === actionId
						);
						if (!isEmpty(currentSideMenu)) {
							...
							console.log(2)
							/* 三级菜单查询 */
							for (let lv3 of lv2.childMenu) {
								if (lv3.childMenu) {
									currentSideMenu = lv3.childMenu.filter(
										leaf => leaf.id === actionId
									);
									return { currentSideMenu};
								}
		...
		filereader.onload = e => {
			...
			image.onload = function() {
				if (width && this.width / this.height !== width / height) {
					// debugger
					Modal.error({
						title: `请上传宽高为 ${width}x${height} 的图片`
          });
          result = false
