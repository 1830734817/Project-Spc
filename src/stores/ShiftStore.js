 ...
 * @LastEditors: shoen
 * @LastEditTime: 2021-05-18 11:08:09
 */
...
class Shift {
	...
	@observable shiftPageInfo = {
		pageIndex: 1,
		pageSize: 10
	};
	@observable shiftStatus = 'add';              // 记录计划弹窗状态
	@observable shiftModalVis = false;
	@observable shiftCurrentRe = {};              // 编辑当前的record
	...
	// isReplenishmentShift=['录入订单','补充订单'];
	...
	/*  ===========生产任务 */
	@action.bound async getShiftList(params) {
		this.isLoading = true;
		this.shiftList = []
		try {
			let res = await services.gets('getShiftList')({
				...omit(this.shiftPageInfo, 'total'),
				...params
			});
			this.isLoading = false;
			if (isDataExist(res)) {
				this.shiftList = res.data.data.items;
				this.shiftPageInfo = res.data.data.page;        // pageIndex: 1, pageSize: 10, total: 17823
			}
		} 
		catch (error) {
			console.log(error);
	...
	@action async save(params) {
		console.log("SAVE");
		try {
			let res = await services.posts('saveShift')(params);
			if (isDataExist(res)) {
				this.queryList();
				return res
			}
	...
	@action async deleteShift(params, type) {
		try {
			let req = type === 'batch' ? 'batchRemove' : 'removeShift';
	...
	@action.bound showModal(visible) {
		this.shiftModalVis = visible;
	}
	
	@action clearModal() {
		this.shiftModalVis = false;
		this.shiftCurrentRe = {}
	}
