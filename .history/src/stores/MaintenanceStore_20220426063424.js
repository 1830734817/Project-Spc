	...
	@action async getDateData(date){
		try {
			this.maintenanceList.map(value =>{
				console.log(value.currentDate,date)
				if(value.currentDate === date){
					const temp= `[${value.state}_${value.equipmentId}_${value.level}_${value.frequency}]`;
					this.consenDateRowName.push(temp);
					this.chosenDateRow.push(value);
					console.log(this.consenDateRowName,this.chosenDateRow)
				}
			})
		} catch (error) {
			console.log(error);
