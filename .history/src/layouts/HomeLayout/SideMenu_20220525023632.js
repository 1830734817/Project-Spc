...
class SideMenu extends Component {
    ...
    console.log(this.store.firstSelectedItem)
	}
  ...
  render() {
    ...
    return (
      <Menu
        ...
        defaultSelectedKeys={this.store.firstSelectedItem}
        mode="inline"
        theme="dark"
        className='sideMenu'
      >
            ...
            firstItem.childMenu && firstItem.childMenu.length > 0 ? (
              //第一层子菜单存在
                <SubMenu key={firstItem.id} title={firstItem.name} 
                         icon={<Dropdown overlay={lv1_subMenu}>
                    ...
                    firstItem.childMenu.map(secondItem =>
                      secondItem.childMenu && secondItem.childMenu.length > 0 ? (
                        ...
                        <SubMenu key={secondItem.id} title={secondItem.name}
                                 icon={<Dropdown overlay={lv2_subMenu}>
                                          <span><SettingOutlined /> </span>
                                        </Dropdown>}
                        >
                          {
                            secondItem.childMenu.map(ele =>
                              <Menu.Item key={ele.id}
                                         icon={<Dropdown overlay={item_menu}>
                                                  <span><SettingOutlined /> </span>
                                                </Dropdown>}
                              >
                                {ele.name}
