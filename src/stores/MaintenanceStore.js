...
class Maintenance{
	...
	@action async saveContentPage(params) {
		...
		try {
			...
			if (isDataExist(res)) {
				...
				this.dataPageModalVis=false;//保养内容页面作为对话框时，提交后需关闭对话框
				this.calendarPageModalVis=false;
	...
	@action async getDateData(date){
			this.chosenDateRowName=[];
			this.chosenDateRow=[];
			const tempRowName=[],tempRow=[];

			try {
				this.maintenanceList.map(value =>{
					if(value.currentDate === date){
						
						tempRowName.push(`[${value.result}]_${value.equipmentId}_${value.level}_${value.frequency}`);
						tempRow.push(value);
					}
				})
				this.chosenDateRowName=tempRowName;
				this.chosenDateRow=tempRow;

			} catch (error) {
				console.log(error);
			}

			console.log(toJS(this.chosenDateRow));
	}
